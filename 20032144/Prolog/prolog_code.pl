...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% How to run %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Option 1. 
% Go to the online SWI Prolog https://swish.swi-prolog.org/
% Copy and paste the code into the editor 
% Type 'play.' into the console
% Click 'Run' to start the game

% Option 2.
% Open the SWI Prolog on your computer
% After the ?- type 'consult(FILEPATH).' where FILEPATH is the path to this file
% Then type 'play.' to start the game
    % For me: consult('/Users/richardaustin/Documents/GitHub/Uni3_ProgLang_Paradigms_Project/20032144/Prolog/prolog_code.pl').
...
% game_loop/0
game_loop :-
    do_we_have_buidlings(BuildingOne, BuildingTwo),     % Check if we have the buildings in memory
    display_both_buildings(BuildingOne, BuildingTwo),   
    check_winner(BuildingOne, BuildingTwo), !,          
    whos_turn(Player),                                  
    take_turn(Player, BuildingOne, BuildingTwo),        
    game_loop.                                          % Loop back to the start of the game

% do_we_have_buidlings(+BuildingOne, +BuildingTwo)
do_we_have_buidlings(NewBuildingOne, NewBuildingTwo) :-
    clause(buildingOne(NewBuildingOne), true),          % Check if we have the building of player one in memory
    clause(buildingTwo(NewBuildingTwo), true).          % Check if we have the building of player two in memory

do_we_have_buidlings(BuildingOne, BuildingTwo) :-
    assert(buildingOne(BuildingOne)),                   % If we dont have the building of player one in memory, assert (store) it
    assert(buildingTwo(BuildingTwo)).                   % If we dont have the building of player two in memory, assert (store) it
